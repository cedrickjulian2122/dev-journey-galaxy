      ...
      <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6 mt-10">
        {skillCategories.map((category, catIndex) => (
          <div 
            key={catIndex}
            className={cn(
              "bg-white dark:bg-navy-light p-6 rounded-lg shadow-sm border border-gray-100 dark:border-navy",
              isVisible ? 'opacity-100 animate-fade-in-up' : 'opacity-0'
            )}
            style={{ animationDelay: `${0.2 + catIndex * 0.1}s` }}
          >
            <h3 className="text-navy dark:text-white font-semibold text-lg mb-4 pb-2 border-b border-gray-100 dark:border-navy">
              {category.name}
            </h3>
                ...
                <li key={skillIndex} className="flex items-center">
                  <div className="w-full">
                    <div className="flex justify-between mb-1">
                      <span className="text-slate-dark dark:text-slate-light">{skill}</span>
                    </div>
                    <div className="w-full bg-gray-200 dark:bg-navy rounded-full h-2">
                      <div 
